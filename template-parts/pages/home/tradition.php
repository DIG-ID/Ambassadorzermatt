<section id="section-tradition" class="section-tradition bg-LightGray relative h-[90dvh] md:min-h-[830] md:max-h-[830] xl:h-[785px] w-full z-20 flex flex-col justify-end md:justify-center">
  ...
  ?>
  <?php
  $bg_id = get_field( 'tradition_image_tablet_mobile' );
  if ( $bg_id ) :
    echo wp_get_attachment_image(
      $bg_id,
      'full',
      false,
      [
        'class' => 'block xl:hidden md:absolute inset-0 w-full h-1/2 md:h-full object-cover -z-10 md:min-h-[440px] object-[0%_15%]',
      ]
    );
  endif;
  ?>
