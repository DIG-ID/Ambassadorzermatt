	...
	<div class="theme-container">
		<div class="theme-grid">
			<div class="col-span-2 md:col-span-4 xl:col-span-5 md:mb-16 xl:mb-0">
				<h2 class="title-secondary text-Dark"><?php the_field( 'pool_sauna_overtitle' ); ?></h2>
				<h3 class="title-main text-Dark md:pt-5 xl:pt-4 md:mb-7 xl:mb-0"><?php the_field( 'pool_sauna_title' ); ?></h3>
				<p class="text-Dark pt-4 md:pt-0 xl:pt-8 pb-10"><?php the_field( 'pool_sauna_description' ); ?></p>
				<?php
				$pslink = get_field( 'pool_sauna_button' );
				if ( $pslink ) :
					$link_url    = $pslink['url'];
					$link_title  = $pslink['title'];
					$link_target = $pslink['target'] ? $pslink['target'] : '_self';
					?>
			...
			<div class="col-span-2 md:col-span-6 xl:col-span-6 col-start-1 md:col-start-1 xl:col-start-7">
				...
				if ( $imghotel1_desktop ) {
					echo wp_get_attachment_image( $imghotel1_desktop, 'full', false, array( 'class' => 'w-full max-h-[55dvh] md:max-h-[60dvh] xl:max-h-none object-cover' ) );
				}
				?>
			</div>
		</div>
