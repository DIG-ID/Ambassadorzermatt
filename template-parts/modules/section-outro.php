...
<section id="section-outro" class="section-outro bg-LightGray relative md:min-h-[830] md:max-h-[830] xl:h-[785px] w-full z-20 flex flex-col justify-end md:justify-center">
	...
	$bg_id_tm = get_field( 'outro_image_tablet_mobile' );
	if ( $bg_id_tm ) :
		echo wp_get_attachment_image( $bg_id_tm, 'full', false, array( 'class' => 'block xl:hidden md:absolute inset-0 w-full h-1/2 md:h-full object-cover -z-10 md:min-h-[440px] object-center' ) );
	endif;
	?>
	<div class="bg-[linear-gradient(#F3F3F3,#F3F3F3)] md:bg-[length:70%_100%] xl:bg-[length:50%_100%] bg-right bg-no-repeat pt-8 pb-10 md:my-16 xl:pt-16 xl:pb-12">
